...
t_flag2=sin(2.*pi*v_freq*t*L_CHAR/U_CHAR)
!t_flag=5.
!t_flag2=5.

lru2=L_char/(rho*U_Char**2) ! to nondimensionalize the forces

if (t_flag .Lt. 0.0 .and. t_flag2 .Lt. 0.0) then
   F_SHAP=1.
   if (nrank .eq. 0) print*,''
...
elseif (t_flag .GE. 0.0 .and. t_flag2 .GE. 0.0) then
   F_SHAP=-1
   if (nrank .eq. 0) print*,''
   if (nrank .eq. 0) print*, '------------ force direction -------- ',F_SHAP,t_flag
   if (nrank .eq. 0) print*,''
