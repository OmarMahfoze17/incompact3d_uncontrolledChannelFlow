...
!############################################################################
  ...
  integer, parameter :: nx1=128 , ny1=129, nz1=84
  integer xitime,itime,ifirst,nx, ny, nz
  real(8),dimension(nx1, ny1, nz1) :: umean,uumean,uvmean
  ...
  real(8),dimension(ny1) :: yp,ypi,qstat,qstat2,qstat_u,qstat_v,qstat_w
  ...
  open (1,file='comu_stat.dat')
      read(1,*) Re,itime,ifirst,nx, ny, nz
if (nx.ne.nx1.or.ny.ne.ny1.or.nz.ne.nz1) then 
print*, '###############################################################'
print*,'                dimension miss match'
print*,nx, ny, nz
print*,nx1, ny1, nz1
print*, '###############################################################'
endif
...
uuz_mean(j,k)/u_to/u_to,uv_mean_2D(j,k)/u_to/u_to
  ...
  do j=1,ny1
     write(10,*) yp(j)*u_to*Re,qstat_u(j)/u_to/u_to,qstat_v(j)/u_to/u_to,qstat_w(j)/u_to/u_to
  enddo
  close(10)
